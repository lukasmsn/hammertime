...
struct ChatView: View {
    ...
    @State private var workoutFilter: Workout? = nil
        ...
        VStack(spacing: 0) {
            ...
            HStack(spacing: 8) {
                TextField("Message", text: $inputText, axis: .vertical)
                    ...
                    .onSubmit { send() }
                Button(action: send) {
                    if isSending { ProgressView().progressViewStyle(.circular) }
                    else { Image(systemName: "paperplane.fill").font(.system(size: 18, weight: .semibold)) }
                }
    ...
    private func send() {
        ...
        let userMsg = Message(role: "user", content: trimmed, workout: workoutFilter)
        context.insert(userMsg)
        try? context.save()

        Task {
            do {
                let contextLines = try ChatContextBuilder.buildMetricsContext(modelContext: context)
                ...
                let assistant = Message(role: "assistant", content: reply, workout: workoutFilter)
    ...
    // Context now provided by ChatContextBuilder
}
